...
if [ "$VERBOSE" = true ]; then
    if docker build -t jira-cloud:local .; then
    ...
    if docker build -t jira-cloud:local . > "$DOCKER_LOG" 2>&1; then
...
echo -e "\n${GREEN}Build complete!${NC} Image tagged as jira-cloud:local"
